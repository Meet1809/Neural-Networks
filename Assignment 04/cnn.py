...
class CNN(object):
    ...
    def add_input_layer(self, shape=(2,),name="" ):
        """
         ...
         """
        #layer number 0
        self.model.add(keras.layers.Input(shape = shape,name=name))
    ...
    def append_dense_layer(self, num_nodes,activation="relu",name="",trainable=True):
        ...
        self.model.add(keras.layers.Dense(num_nodes,activation=activation,trainable=trainable,name=name))
    ...
    def append_conv2d_layer(self, num_of_filters, kernel_size=3, padding='same', strides=1,
        ...
        # self.weights = initializer(shape=kernel_size)
        # self.biases = initializer(shape=(num_of_filters,1))
        # print(self.weights)
        
        x = keras.layers.Conv2D(filters= num_of_filters,kernel_size= kernel_size,padding=padding,strides=strides,activation=activation,name=name,trainable=trainable)
        self.model.add(x)
        return x
    ...
    def append_maxpooling2d_layer(self, pool_size=2, padding="same", strides=2,name=""):
        """
         ...
         :return: Layer object
         """
        x = keras.layers.MaxPooling2D(pool_size=pool_size,padding=padding,strides= strides,name="name")
        self.model.add(x)
        return x
    ...
    def append_flatten_layer(self,name=""):
        """
         ...
         :return: Layer object
         """
        x = keras.layers.Flatten(name=name)
        self.model.add(x)
        return x
    ...
    def get_weights_without_biases(self,layer_number=None,layer_name=""):
        ...
        This means that the first layer with activation function is layer zero
         ...
         """
        temp_layer_no = 1
        if layer_number != None and layer_name != "":
            #layer number will precede layer name
            if layer_number == 0:
                #input layer
                return None
            else:
                for i in self.model.layers:
                    if temp_layer_no == layer_number:
                        return i.get_weights()[0]
                    temp_layer_no = temp_layer_no + 1

        elif layer_number == None:
            if layer_name in self.model.layers:
                if len(self.model.get_layer(layer_name).get_weights()) > 0:
                    w = self.model.get_layer(layer_name).get_weights()[0]
                    return w
    ...
    def get_biases(self,layer_number=None,layer_name=""):
        ...
        if layer_number != None and layer_name != "":
            ...
            if layer_number == 0:
                ...
                for i in self.model.layers:
                    if temp_layer_no == layer_number:
                        return i.get_weights()[1]
                    temp_layer_no = temp_layer_no + 1

        elif layer_number == None:
            if layer_name in self.model.layers:
                if len(self.model.get_layer(layer_name).get_weights()) > 0:
                    w = self.model.get_layer(layer_name).get_weights()[1]
                    return w
                else:
                    return None
        ...
        if layer_number == 0:
            ...
            for i in self.model.layers:
                if temp_layer_no == layer_number:
                    lst = []
                    lst.append(weights)
                    lst.append(self.get_biases(layer_number))
                    i.set_weights(lst)
                    temp_layer_no = temp_layer_no + 1
                else:
                    temp_layer_no = temp_layer_no + 1
    ...
    def set_biases(self,biases,layer_number=None,layer_name=""):
        ...
        if layer_number == 0:
            ...
            temp_layer_no = 1
            for i in self.model.layers:
                print("temp_layer number ",temp_layer_no)
                if temp_layer_no == layer_number:
                    lst = []
                    lst.append(biases)
                    # lst.append(self.get_biases(layer_number))
